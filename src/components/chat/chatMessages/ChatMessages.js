...
export default function ChatMessages(props) {
    ...
    useEffect(() => {
        const connect = new HubConnectionBuilder()
            .withUrl("http://localhost:7038/myHub")
    ...
    useEffect(() => {
        async function getMessages(){
            (async () => {
                const response = await fetch(`http://localhost:7038/api/contacts/${props.contactUserName}/messages?user=${props.logInUserName}`);                
    ...
    async function UpdateMyServer(){
        ...
        const init = {
         ...
         await fetch(`http://localhost:7038/api/contacts/${props.contactUserName}/messages`, init);
    }
    ...
    async function UpdateContactServer(){
        ...
        var server= props.contactServer;
        var x=server.split(":");
        if(x[0]==("10.0.2.2")){
            x[0]="localhost";
            server=x.join(":")
        }
        await fetch(`http://${server}/api/transfer`, init);
    }
